    ...
    fs.readFile(settingPath, "utf-8", (err, data) => {
        if (err) {
            ...
            if (!webContents.isDestroyed()) {
                webContents.send("LiteLoaderQQNT.momotalk_theme.updateSetting", setting[getCurrTheme()]);
            }
        }
    });
}
...
function updateCSS(webContents, cssPath) {
    fs.readFile(cssPath, "utf-8", (err, data) => {
        if (err) {
            ...
            webContents.send("LiteLoaderQQNT.momotalk_theme.updateCSS", data);
        }
    });
}
...
function onLoad(plugin) {
    ...
    ipcMain.on("LiteLoaderQQNT.momotalk_theme.rendererReady", async (event, message) => {
    ...
    ipcMain.handle('LiteLoaderQQNT.momotalk_theme.getSetting', async (event, message) => {
        return getSetting();
    });
    ipcMain.on('LiteLoaderQQNT.momotalk_theme.setSetting', async (event, message) => {
        return setSetting(message);
    });
    ipcMain.on('LiteLoaderQQNT.momotalk_theme.setWallpaper', async (event, message) => {
        setWallpaper();
    });
    ipcMain.on('LiteLoaderQQNT.momotalk_theme.resetSetting', async (event, message) => {
        initSetting(true);
    });
}
